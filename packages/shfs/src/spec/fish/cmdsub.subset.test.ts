...
test('cmdsub subset: command substitution concatenated with double-quoted segments', async () => {
	expect(await run('echo "a"(echo b)"c"')).toBe('abc');
});

// cmdsub.fish: echo "$(echo "$(echo a)")"
// Adapted: nested substitution, unquoted.
test('cmdsub subset: nested command substitution', async () => {
...
test('cmdsub subset: multiple command substitutions joined by quoted literals', async () => {
	expect(
		await run(
...
test('cmdsub subset: substitution adjacent to quoted trailing paren', async () => {
...
test('cmdsub subset: quoted literal parens surrounding a substitution', async () => {
	expect(await run('echo "("(echo 1)")"')).toBe('(1)');
});

// cmdsub.fish: echo "$(echo 1) ( $(echo 2)"
// Adapted: substitution + quoted literal " ( " + substitution.
test('cmdsub subset: literal paren between substitutions', async () => {
...
test('cmdsub subset: mixed quoted and unquoted segments with substitutions', async () => {
...
test('cmdsub subset: quoted literal parens wrapping substitutions and text', async () => {
...
test('cmdsub subset: adjacent quoted segments and unquoted substitutions', async () => {
	expect(
		await run(
...
test('cmdsub subset: variable expansion adjacent to command substitution', async () => {
...
test('cmdsub subset: escaped dollar is literal before command substitution', async () => {
	expect(await run('echo \\$(echo 1)')).toBe('$1');
});
...
test('cmdsub subset: bare parentheses inside double quotes are literal', async () => {
...
test('cmdsub subset: nested parens inside double quotes are literal', async () => {
	expect(await run('echo "fn(a, b(c))"')).toBe('fn(a, b(c))');
});

test('cmdsub subset: empty parens inside double quotes are literal', async () => {
	expect(await run('echo "init()"')).toBe('init()');
});

// cmdsub.fish: parentheses inside single quotes are always literal.
test('cmdsub subset: parentheses inside single quotes are literal', async () => {
	expect(await run("echo 'hello(world)'")).toBe('hello(world)');
});

// Basic unquoted command substitution.
test('cmdsub subset: basic unquoted command substitution', async () => {
	expect(await run('echo (echo hello)')).toBe('hello');
});

// Deeply nested command substitution.
test('cmdsub subset: deeply nested command substitution', async () => {
