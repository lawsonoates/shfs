...
test('gnu grep: char-class-multibyte + char-class-multibyte2 - accented characters remain stable in classes and groups', async () => {
	...
	const invalid = await harness.runWithStatus(
		`grep ${quote('[é]')} /tmp/invalid-byte.txt`
	);
